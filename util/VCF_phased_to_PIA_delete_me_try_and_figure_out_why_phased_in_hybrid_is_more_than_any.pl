...
foreach my $file(@files) {
	...
	VCF1: while (my $line = <$fh>) {
		...
		SAMPLE: for(my $i=0; $i < $number_of_samples; $i++) {
			...
			#warn "phased) $contig $position $phased_id $$VCF_line{$phased_id}\n";
			...
			warn "phased) $contig $position $phased_id $$VCF_line{$phased_id}\n";
			#warn "i have info for phased position\n";
			...
			for(my $i=$first_phase_group_position; $i<=$last_phase_group_position;$i++) { 
				if($opt_p eq 1) { 
					#warn "$$VCF_line{'supercontig'} -> $i = 1\n";
					@{$$genome_coverage{$$VCF_line{'supercontig'}}}[$i] = '1'; }
				else { 
					@{$$genome_coverage{$$VCF_line{'supercontig'}}}[$i]++; 
					#warn "$$VCF_line{'supercontig'} -> $i ++ = @{$$genome_coverage{$$VCF_line{'supercontig'}}}[$i]\n";
				}
			}
...
warn "\n\nIdentified haplotypes on " . scalar(keys(%haplotype_regions)) . " supercontigs\n";
...
CONTIGS: foreach my $contig(sort keys %{$genome_coverage}) {
	...
	for(my $i=0; $i<$length;$i++) {
		...
		if($test[$i] eq $number_of_files_needed) {
			## Found start
			if($am_i_searchin eq 0) { 
				warn "start of no alignment = $i\n";
				$start_of_no_alignment = ($i+1); 
			}
			$am_i_searchin=1;
		}
		...
		if((($opt_p eq 1) && (($test[$i] eq 0) || ($i eq ($length-1)))) ||
		   (($opt_p eq 2) && (($test[$i] < $number_of_files_needed) || ($i eq ($length-1))))) {
			warn "end of phased region? $opt_p and $test[$i] and $number_of_files_needed and $i and $length\n";
		   	$am_i_searchin=0;
